...
#-----------------------------------------------------------------------
def stackImages(data_path, data_type, flat_path, dark_path, dx_list, dy_list, valid_list):

    #---- Chargement des meta-donnees
    if data_type == "SER":
        print("INFO   : Traitement de donnees SER")

        #--- Chargement du header SER
        header = utils.readSerHeader(data_path)
        print("INFO   :", header['FrameCount'], "images a traiter")

        #---- Ouverture du fichier
        file = open(data_path, 'rb')

    elif data_type == "FITS":
        print("INFO   : Traitement de donnees FITS")

        #--- Recherche des images dans le repertoire
        im_list = glob.glob(os.path.join(data_path, "*.fit*"))

        header = {}
        header['FrameCount'] = len(im_list)

    else:
        print("ERREUR : Type de donnees", data_type, "inconnu")
        sys.exit(2)
    ...
    for i in range(len(valid_list)):
        ...
        if valid_list[i] == 1:
            ...
            if data_type == "SER":
                ...
                currentIm, nb_channel = utils.loadImFitsFloat64(im_list[i])
...
#-----------------------------------------------------------------------
def computeShift(data_path, data_type, flat_path, dark_path, score_min):
    ...
    for i in range(header['FrameCount']):
        ...
        #print("DEBUG  : currentIm", currentIm.shape, currentIm.dtype)
        ...
        if i == 0:
            ...
            norm = Normalize(vmin=bgMean-6*bgSigma, vmax=bgMean+10*bgSigma)
            norm = Normalize(vmin=1000, vmax=1200)
            ...
            print("INFO   : Seuillage, bgMean", bgMean, ", bgSigma", bgSigma, ", seuil", seuil)
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="python stackJupiter.py",
        description="Outil de stacking pour des images de Jupiter")
    
    parser.add_argument("-i", "--input_path", type=str, required=True, default=None,
        help="(Obligatoire) Chemin vers le fichier SER")
    
    parser.add_argument("-t", "--type_data", type=str, required=True, default=None,
        help="(Obligatoire) Type de donnees : SER ou FITS")
    ...
    #---- Calcul des decalages
    score_min = 0.989
    dx_list, dy_list, score_list, valid_list = computeShift(args.input_path, args.type_data, args.input_flat, args.input_dark, score_min)
    ...
    #---- Stacking
    finale_image, first_image = stackImages(args.input_path, args.type_data, args.input_flat, args.input_dark, dx_list, dy_list, valid_list)


    fig, ax = plt.subplots(1, 2, sharex=True, sharey=True)
    norm = Normalize(vmin=1000, vmax=1300)
    ...
    if args.output_path is not None:
        ...
        if args.type_data == "FITS":

            finale_image_tmp = copy.deepcopy(finale_image)
            finale_image_tmp[:,:,0] = finale_image[:,:,2]
            finale_image_tmp[:,:,2] = finale_image[:,:,0]
            finale_image = copy.deepcopy(finale_image_tmp)
